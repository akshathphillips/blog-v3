...
Each worker does one thing, leaves a note about what it did, and doesn't know the others exist. The notes are the conveyor belt: a little status on each record, *queued, submitted, ready, downloaded, written*.
...
**Make the state visible, not implied.** Every record should carry its own answer to "where am I in the process?" That answer belongs somewhere durable, not in a running program's memory. Then a crash costs you nothing: restart, read the notes, continue. Nobody has to remember anything.
